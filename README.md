...
ST7735S 控制器 GRAM 可为 132×162，而常见玻璃可视区只有 128×160。
原驱动从控制器 `(0,0)` 写到 `(127,159)`，本块模组实际可视原点为
`(1,1)`，所以可视区最右一列和最下一行没有被覆盖，保留为彩线。

当前修复在 `drivers/inc/st7735.h`：
...
所有 `CASET/RASET` 地址窗口都会自动加入偏移，`FillScreen()` 现在写入
GRAM `(1,1)` 到 `(128,160)`。字符绘制也已改为每个字符只设置一次窗口，
动态刷新不再为每个像素重复发送地址命令。

若更换另一种 tab/玻璃批次后仍有边线，优先分别试验 `(2,1)`、`(0,0)`，
不要把逻辑宽高改成 129×161。
